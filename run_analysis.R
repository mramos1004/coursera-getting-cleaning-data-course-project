...
#function for downloading the data initially.
...
#function for merging data sets and test sets
...
#function for extracting mean and standard deviation
...
#function ofr describing activities to be descriptive
...
#grouped function to iterate all steps using the functions above
clean.data = function() {
    # Download data
    download.data()
    ...
    message("Write tidy dataset as csv")
    write.csv(tidy, "UCI_HAR_TIDY_DATA_SET.csv", row.names=FALSE)
    f <- file.path(getwd(), "UCI_HAR_TIDY_DATA_SET.txt")
    message("Write tidy dataset as txt file in tab separated values")
